...
function buildDAO() {
  ...
  [ ! -d "$EXTROOT/CRM/Membersonlyevent/DAO/" ] && mkdir -p "$EXTROOT/CRM/Membersonlyevent/DAO/"
  cp -f "$CIVIROOT/CRM/Membersonlyevent/DAO"/* "$EXTROOT/CRM/Membersonlyevent/DAO/"
}

##############################
function cleanup() {
  for DIR in "$XMLBUILD" "$CIVIROOT/CRM/Membersonlyevent" "$EXTROOT/CRM/Membersonlyevent/DAO/" ; do
    if [ -e "$DIR" ]; then
      rm -rf "$DIR"
    fi
  done
